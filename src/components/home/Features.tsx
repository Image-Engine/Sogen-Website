...
export function Features() {
  ...
  return <section className="relative py-16 sm:py-20 lg:py-32 overflow-hidden" style={{
    ...
    backgroundAttachment: "fixed"
      ...
      <div className="container max-w-6xl relative z-10 px-4 sm:px-6">
        ...
        <div className="mb-10 sm:mb-14 lg:mb-20">
          ...
          <div className="flex justify-center items-stretch gap-4 sm:gap-6 lg:gap-8 max-w-4xl mx-auto mb-6" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
            ...
            return <button key={cardIndex} onClick={() => setCurrentIndex(cardIndex)} className={`relative flex-1 group rounded-2xl sm:rounded-3xl backdrop-blur-xl text-center p-4 sm:p-6 lg:p-8 bg-white/5 border border-white/10 transition-transform duration-500 ease-out ${isCenter ? 'scale-105 sm:scale-110 z-10' : 'scale-90 sm:scale-95'}`}>
                  ...
                  <div className="relative z-10">
                    ...
                    <div className={`font-semibold mb-1 text-xs sm:text-sm lg:text-base transition-colors duration-500 ${isCenter ? 'text-amber-200' : 'text-white/50'}`}>
                      {stat.subtitle}
                    </div>
                    <div className={`text-[10px] sm:text-sm transition-opacity duration-500 ${isCenter ? 'opacity-100 text-white/70' : 'opacity-0'}`}>
                      {stat.desc}
                    </div>
                  </div>
                </button>;
          ...
          <div className="flex justify-center gap-2 mb-8 sm:mb-10" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
            {heroCarouselData.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`h-2 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-amber-400 w-6' : 'bg-white/40 w-2 hover:bg-white/60'}`} aria-label={`Go to slide ${index + 1}`} />)}
          </div>
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="space-y-3">
              ...
              return <div key={item.title} className="group flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-white/10 hover:bg-white/15 border border-white/15 hover:border-white/30 transition-colors duration-300">
        ...
        <div>
          ...
          stopOnInteraction: true
            ...
            <CarouselContent className="-ml-3 sm:-ml-4">
              {features.map((item, index) => <CarouselItem key={index} className="pl-3 sm:pl-4 basis-full sm:basis-1/2 lg:basis-1/3">
